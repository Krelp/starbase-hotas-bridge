...
DEFAULT_PROFILE = {
    ...
    "movements": {
        "yaw":       {"physical_axis": 5,       "key_left": "q",    "key_right": "e",    "deadzone": 15, "speed": 70, "pulse": 10, "inverted": True,  "control_points": [[0.125, 0.0199], [0.25, 0.0841], [0.375, 0.1913], [0.5, 0.2973], [0.625, 0.4031], [0.75, 0.5199], [0.875, 0.6894]]},
        "pitch":     {"physical_axis": 1,       "key_left": "w",    "key_right": "s",    "deadzone": 15, "speed": 70, "pulse": 12, "inverted": False, "control_points": [[0.125, 0.2333], [0.25, 0.3789], [0.375, 0.5033], [0.5, 0.6156], [0.625, 0.7196], [0.75, 0.8176], [0.875, 0.9108]]},
        "roll":      {"physical_axis": 0,       "key_left": "a",    "key_right": "d",    "deadzone": 15, "speed": 70, "pulse": 8,  "inverted": False, "control_points": [[0.125, 0.0199], [0.25, 0.0841], [0.375, 0.1913], [0.5, 0.2973], [0.625, 0.4031], [0.75, 0.5199], [0.875, 0.6894]]},
...
class MovementCard(QWidget):
    ...
    def _build(self):
        ...
        not_used_btn.setToolTip(
            "Mark this movement as unused.\n"
            "Use this if you don't have a device for this axis\n"
            "(e.g. no pedals, single stick, etc.)")
        ...
        self.dz_sl,dz_col=sl_row("DEAD ZONE",
            ...
            5, 40, max(5, min(40, self.cfg.get("deadzone",15))),
            lambda v: f"{'min' if v<=5 else 'small' if v<15 else 'medium' if v<25 else 'large'} ({v}%)")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Adapt to screen size — cap at 95% of screen width/height
        from PyQt6.QtWidgets import QApplication as _QApp
        screen = _QApp.primaryScreen().availableGeometry()
        win_w = min(WIN_W, int(screen.width() * 0.95))
        win_h = min(WIN_H, int(screen.height() * 0.95))
    ...
    def _load_startup_profile(self):
        # Check if this is genuinely a first launch BEFORE we copy anything
        session_exists  = SESSION_STATE_FILE.exists()
        profiles_exist  = len(list_profiles()) > 0
        is_first_launch = not session_exists and not profiles_exist
        ...
        if profiles:
            # If this is a true first launch, show welcome even though we
            # just copied the bundled profile — user hasn't chosen anything yet
            if is_first_launch:
    ...
    def _build_ui(self):
        ...
        ver_lbl.setStyleSheet("color:rgba(180,160,255,80);font-size:11px;")
        tsl.addWidget(ver_lbl)
        tsl.addSpacing(8)
        ...
        self.turtle_btn.setStyleSheet(
            ...
            "border:2px solid rgba(120,100,200,80);}"
            "QPushButton:checked{border:2px solid #f0c030;color:#f0c030;}"
            "QPushButton:hover{border-color:#f0c030;}")
    ...
    def _build_turtle_banner(self):
        ...
        w.setStyleSheet(
            "background:qlineargradient(x1:0,y1:0,x2:1,y2:0,"
            "stop:0 rgba(20,15,40,180),stop:0.5 rgba(28,20,52,180),stop:1 rgba(20,15,40,180));"
            "border:1px solid rgba(160,140,220,40);border-radius:10px;")
        ...
        desc.setStyleSheet("font-size:13px;background:transparent;border:none;")
        ...
        cfg_btn.setStyleSheet(
            ...
            "border:1px solid rgba(160,140,220,80);border-radius:6px;padding:0 14px;}"
            "QPushButton:hover{border-color:rgba(240,192,48,180);}")
        ...
        tog.setStyleSheet(
            ...
            "border:2px solid rgba(120,100,200,80);border-radius:6px;padding:0 14px;}"
            "QPushButton:checked{border:2px solid #f0c030;color:#f0c030;}"
            "QPushButton:hover{border-color:#f0c030;}")
